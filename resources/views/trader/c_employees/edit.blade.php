        ...
        <div class="col-lg-12 col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{route('c_employees.update',$c_employee->id)}}" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                        @method('put')
                            ...
                            <div class="col-lg-12 col-xl-12">
                                <div class="card card-dashboard-map-one">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" value="{{old('name',$c_employee->name)}}" class="form-control" id="name" name="name" required>
                                        @error('name')<span class="text-danger">{{ $message }}</span>@enderror
                                    </div>
                                    <br>
                                    <div>
                                        <label for="exampleInputEmail1">البريد الألكتروني : <span class="tx-danger">*</span> </label>
                                        <input type="email" value="{{old('email',$c_employee->email)}}" class="form-control" id="email" name="email" required>
                                        @error('email')<span class="text-danger">{{ $message }}</span>@enderror
                                    </div>
                                    <br>
                                    <div>
                                        <label for="exampleInputEmail1">رقم الهاتف : <span class="tx-danger">*</span> </label>
                                        <input type="text" value="{{old('phone',$c_employee->phone)}}" class="form-control" id="phone" name="phone" required>
