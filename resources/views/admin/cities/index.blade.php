    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    @if (str_contains(auth('admin')->user()->permissions, "add_city") !== false)
                    <div class="col-sm-1 col-md-2">
                        <a class="btn btn-primary modal-effect" href="#modaldemo8" data-toggle="modal">اضافة محافظة</a>
                    </div>
                    @endif
                    @include('admin.cities.filter.filter')
                </div>
                <div class="card-body">
                    <div class="table-responsive hoverable-table">
                        <table class="table table-hover" id="example1" data-page-length='50' style=" text-align: center;">
                            ...
                            <tr>
                                ...
                                @if (str_contains(auth('admin')->user()->permissions, "delete_city") !== false || str_contains(auth('admin')->user()->permissions, "edit_city") !== false)
                            ...
                            @forelse ($cities as $key => $city)
                                <tr>
                                    ...
                                    @if (str_contains(auth('admin')->user()->permissions, "delete_city") !== false || str_contains(auth('admin')->user()->permissions, "edit_city") !== false)
                                    <td>
                                        @if (str_contains(auth('admin')->user()->permissions, "edit_city") !== false)
                                        <a href="{{ route('cities.edit', $city->id) }}" class="btn btn-sm btn-info"
                                           title="تعديل"> <i class="las la-pen"></i> تعديل</a>
                                        @endif
                                        @if (str_contains(auth('admin')->user()->permissions, "delete_city") !== false)
                                        <button class="btn btn-danger btn-sm " data-id="{{ $city->id }}"
                                                data-name_ar="{{ $city->name_ar }}" data-toggle="modal"
                        ...
                        <br><div class="text-center">{!! $cities->links('layouts.pagination') !!}</div>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">اضافة المحافظة</h6><button aria-label="Close" class="close"
                                                                         data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <form action="{{ route('cities.store') }}" class=parsley-style-1' method="post" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal" id="modaldemo9">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">حذف المحافظة</h6><button aria-label="Close" class="close"
                                                                         data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                    </div>
                    @if($cities->count() != 0)
