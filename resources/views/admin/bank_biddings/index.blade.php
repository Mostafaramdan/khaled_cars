...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">مزادات البنوك</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"></span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    ...
                    @endif
                    @include('admin.bank_biddings.filter.filter')
                </div>
                <div class="card-body">
                    <div class="table-responsive hoverable-table">
                        <table class="table table-hover" id="example1" data-page-length='50' style=" text-align: center;">
                            ...
                            <tbody>
                            @if(isset($bank_biddings))
                            @forelse($bank_biddings as $key => $bank_bidding)
                                <tr>
                                    <td>{{ $bank_bidding->product->name_ar }}</td>
                                    <td>{{ $bank_bidding->Insurance }}</td>
                                    <td>{{ $bank_bidding->min_auction }}</td>
                                    <td>{{ $bank_bidding->bank->name }}</td>
                                    @if($bank_bidding->type == 'open')
                                            <td>
                                                <form action="{{route('bank_biddings.status',$bank_bidding->id)}}" method="post">
                                                    {{csrf_field()}}
                                                    @method('put')
                                    ...
                                    <td>
                                        <a href="{{ route('bank_biddings.show', $bank_bidding->id) }}" class="btn btn-sm btn-warning"
                                           title="عرض"> <i class="las la-eye"></i> عرض </a>
                                        @if (str_contains(auth('admin')->user()->permissions, "edit") !== false)
                                        <a href="{{ route('bank_biddings.edit', $bank_bidding->id) }}" class="btn btn-sm btn-info"
                                           title="تعديل"> <i class="las la-pen"></i> تعديل</a>
                                        @endif
                                        @if (str_contains(auth('admin')->user()->permissions, "delete") !== false)
                                        <button class="btn btn-danger btn-sm " data-id="{{ $bank_bidding->id }}"
                                                data-name_ar="{{ $bank_bidding->product->name_ar }}" data-toggle="modal"
                        ...
                        <br><div class="text-center">{!! $bank_biddings->links('layouts.pagination') !!}</div>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="modal" id="modaldemo8">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    ...
                    <form action="{{ route('bank_biddings.store') }}" class=parsley-style-1' method="post" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal" id="modaldemo9">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    ...
                    @if($bank_biddings->count() != 0)
                        <form action="bank_biddings/destroy" method="post">
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
                            <div class="modal-body">
